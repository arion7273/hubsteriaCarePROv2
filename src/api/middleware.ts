...
export function redactBody(body: unknown): unknown {
  ...
  return Object.fromEntries(
    Object.entries(body as Record<string, unknown>).map(([key, value]) => [
      key,
      ['password', 'code', 'token', 'apiKey', 'secret', 'firstName', 'lastName', 'preferredName', 'dob', 'dateOfBirth', 'ssn', 'diagnosis', 'medication', 'room'].some((sensitive) => key.toLowerCase().includes(sensitive.toLowerCase()))
        ? '[REDACTED]'
        : value
    ])
  );
