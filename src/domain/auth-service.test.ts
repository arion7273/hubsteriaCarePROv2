...
describe('AuthService', () => {
  ...
  it('locks accounts after repeated failed login attempts and audits each attempt', async () => {
    const { repositories, service } = createAuthService();
    await repositories.users.save(activeT3User);

    for (let attempt = 0; attempt < 5; attempt += 1) {
      await expect(service.login({ email: activeT3User.email, password: 'wrong-password' })).rejects.toThrow(
        'Invalid credentials'
      );
    }
  ...
  it('completes password reset by updating credentials, marking request used, clearing lockout, and auditing', async () => {
    const { repositories, service } = createAuthService();
    await repositories.users.save(activeT3User);
    await expect(service.login({ email: activeT3User.email, password: 'wrong-password' })).rejects.toThrow(
      'Invalid credentials'
    ...
    await service.completePasswordReset({ requestId: reset.id, newPassword: 'new-correct-password' });
