...
export class AuthService {
  ...
  async completePasswordReset(input: { requestId: UUID; newPassword: string }): Promise<void> {
    const reset = await this.repositories.passwordResets.getById(input.requestId);
    ...
    if (reset.usedAt) {
      throw new Error('Password reset request already used');
    }

    if (Date.parse(reset.expiresAt) < now.getTime()) {
      throw new Error('Password reset request expired');
    }

    const user = await this.requireUser(reset.userId);
    ...
    const completed = await this.repositories.passwordResets.save({ ...reset, usedAt: now.toISOString() });
    const existingSecurityState = await this.repositories.accountSecurity.getByUserId(user.id);
    if (existingSecurityState) {
      await this.resetAccountSecurity(user, existingSecurityState, now);
    }
    ...
    await this.repositories.auditLogs.append(
      createAuditEvent({
        ...
        scope: { scope: user.organizationId ? 'organization' : 'platform', organizationId: user.organizationId },
        beforeState: { usedAt: reset.usedAt },
